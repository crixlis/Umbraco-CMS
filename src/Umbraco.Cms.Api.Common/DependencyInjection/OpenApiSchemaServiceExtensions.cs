...
{
    private const string OpenApiSchemaServiceName = "OpenApiSchemaService";
    private static readonly string? _openApiAssemblyName = typeof(OpenApiOptions).Assembly.GetName().Name;
        ...
        string jsonOptionsName)
    {
        ServiceDescriptor descriptor = services.FirstOrDefault(sd =>
            sd.ServiceType.Assembly.GetName().Name == _openApiAssemblyName
            && sd.ServiceType.Name == OpenApiSchemaServiceName
            && Equals(sd.ServiceKey, documentName))
            ?? throw new InvalidOperationException(
                $"Could not find a registration for {OpenApiSchemaServiceName} (assembly {_openApiAssemblyName}) keyed with '{documentName}'. "
